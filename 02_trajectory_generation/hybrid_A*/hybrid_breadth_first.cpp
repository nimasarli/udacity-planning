...
double HBF::heuristic(double x, double y, vector<int> &goal){
  // Manhattan distance 
  return fabs(y - goal[1]) + fabs(x - goal[0]); //return grid distance to goal
}
...
// Hybrid A* is essentially A* with added logic to include car kinematic.
// Here, a RWD bicycle model is used. 
// Main function to call 
HBF::maze_path HBF::search(vector< vector<int> > &grid, vector<double> &start, 
                           vector<int> &goal) {
  // closed is similar to "finalized" set in A*
  ...
  int total_closed = 1;
  // Similar to Open set in A* 
  vector<maze_s> opened = {state};
  // bool finished = false;
  while(!opened.empty()) {
    sort(opened.begin(), opened.end(), compare_maze_s); // can use priority queue instead
    ...
    // expand is analogous to finding adjacent vertices of the current vertices in A*
